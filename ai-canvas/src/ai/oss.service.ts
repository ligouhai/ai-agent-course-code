...
import { OssClient, OssClientConstructor } from './oss-client.interface';
...
export class OssService {
  ...
  private readonly postClient: OssClient;
  private readonly bucket: string;
  private readonly region: string;

  constructor(private readonly config: ConfigService) {
    const OssClientClass = OSS as unknown as OssClientConstructor;
    ...
    this.client = new OssClientClass({
      region: this.region,
      accessKeyId,
      accessKeySecret,
    ...
    this.postClient = new OssClientClass({
      region: this.region,
      accessKeyId,
      accessKeySecret,
      bucket: this.bucket,
  ...
  private wrapOssError(error: unknown, action: string): Error {
    ...
    this.logger.error(
      `OSS ${action} failed: ${ossError.code ?? 'UnknownError'} - ${ossError.message ?? String(error)}`,
    );
