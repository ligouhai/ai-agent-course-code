...
import { OssService, OssUploadSignature } from './oss.service';
...
export class AiService {
  ...
  getUploadSignature(ext?: string): OssUploadSignature {
  ...
  private async generateByEdit(
    ...
    const resultUrl = this.extractImageUrl(raw);
    if (!resultUrl) {
      throw new BadRequestException(
        `No image URL in DashScope response: ${JSON.stringify(raw)}`,
      );
    }
