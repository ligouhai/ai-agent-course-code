/*
 * @Date: 2026-07-22 14:37:49
 * @LastEditors: zhujinyi
 * @LastEditTime: 2026-07-22 15:05:20
 */
...
async function main() {
  ...
  //   pageSize: 10,
  // });
  // log("所有记忆", allMemories);

  // const firstMemory = allMemories.results?.[0] ?? searchResults.results?.[0];
  ...
  if (process.argv.includes("--cleanup")) {
    const deleted = await client.deleteAll({ user_id: USER_ID });
    log("删除所有记忆", deleted);
  } else {
    console.log(
      "\n 提示：运行 `node mem0-test.mjs --cleanup` 可删除本次测试用户全部记忆",
    );
  }
}
