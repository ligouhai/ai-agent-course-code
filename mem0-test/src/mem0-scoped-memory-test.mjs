...
async function searchUserMemory(client) {
  const searched = await client.search("用户住哪里，有什么爱好", {
    filters: {
      user_id: USER_ID,
    },
    topK: 5,
  });
  log("用户记忆 -- search", searched.results?.map((item) => item.memory) ?? []);
}
...
async function searchSessionMemory(client) {
  ...
  log("会话记忆 -- search", searched.results?.map((item) => item.memory) ?? []);
  ...
  log("会话记忆 -- getAll", listed.results?.map((item) => item.memory) ?? []);
}
...
async function searchAgentMemory(client) {
  ...
  log(
    "Agent 记忆 -- search",
    searched.results?.map((item) => item.memory) ?? [],
  );
  ...
  log("Agent 记忆 -- getAll", listed.results?.map((item) => item.memory) ?? []);
}

async function main() {
  if (!process.env.MEM0_API_KEY) {
    console.error("缺少 Mem0 API Key");
    process.exit(1);
  }
  ...
  if (process.argv.includes("--cleanup")) {
    ...
    log("所有记忆已清除", { USER_ID, RUN_ID, AGENT_ID });
    return;
  }
  if (action === "add") {
