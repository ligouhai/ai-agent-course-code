...
const MEMORY_TTL_SECONDS = Number(process.env.MEMORY_TTL_SECONDS || 1800);
const MEMORY_KEY_PREFIX = process.env.MEMORY_KEY_PREFIX || "agent:short_memory";
const MEMORY_SESSION_ID = process.env.MEMORY_SESSION_ID || "session_001";
const MEM0_USER_ID = process.env.MEM0_USER_ID || "demo_user_001";
...
const memorySchema = z.object({
  write_user: z
    .boolean()
    .describe(
      "写入用户层：换一个新对话仍然保留的长期事实（身份、居住地、长期爱好、饮食禁忌、持久偏好）。不含仅本轮任务。",
    ),
  write_session: z
    .boolean()
    .describe(
...
class Mem0MemoryStore {
  ...
  buildSystemMessage({ user, session }) {
    ...
    if (user.length) {
      blocks.push(
        `【用户长期记忆】\n${user.map((item) => item.memory).join("\n")}`,
      );
    }
    if (session.length) {
      blocks.push(
        `【当前会话记忆】\n${session.map((item) => item.memory).join("\n")}`,
      );
    }
...
async function invokeWithMem0(
  ...
  if (mem.user.length)
    console.log(`  ↳ Mem0 用户记忆命中 ${mem.user.length} 条`);
  if (mem.session.length)
    console.log(`  ↳ Mem0 会话记忆命中 ${mem.session.length} 条`);
  ...
  const assistantText = result.messages[result.messages.length - 1].content;
...
if (!process.env.MEM0_API_KEY || !process.env.OPENAI_API_KEY) {
  console.error("MEM0_API_KEY 或 OPENAI_API_KEY 未设置");
  process.exit(1);
}
...
try {
  ...
  console.error("Redis 未连接，请先执行：docker compose up -d");
  process.exit(1);
}
...
console.log(`用户 ${MEM0_USER_ID} | 会话 ${MEMORY_SESSION_ID}`);
console.log(
...
let prevCount = (await redisStore.loadMessages(MEMORY_SESSION_ID)).length;
...
try {
  while (true) {
    ...
    if (userText === ":clear") {
      await redisStore.clear(MEMORY_SESSION_ID);
      prevCount = 0;
    ...
    const { redisMessages, assistantText } = await invokeWithMem0(
      agent,
      redisStore,
      mem0Store,
      MEMORY_SESSION_ID,
