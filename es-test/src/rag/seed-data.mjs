/*
 * @Date: 2026-06-26 14:34:21
 * @LastEditors: zhujinyi
 * @LastEditTime: 2026-06-26 15:01:50
 */
...
async function seedElasticsearch(indexName, rows) {
  try {
    console.log("Rebuilding ES index and bulk inserting data...");
    ...
    console.log(`写入${rows.length}条数据`);
...
async function seedMilvus(collectionName, rows, emb) {
  try {
    ...
    if (hasCollection.value) {
      console.log(`删除已有集合: ${collectionName}`);
      await milvusClient.deleteCollection({
        collection_name: collectionName,
      });
      console.log("集合删除成功");
    }
