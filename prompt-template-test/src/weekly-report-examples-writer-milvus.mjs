/*
 * @Date: 2026-05-19 18:57:57
 * @LastEditors: zhujinyi
 * @LastEditTime: 2026-05-20 11:44:38
 */
  ...
  try {
    ...
    const insertData = await Promise.all(
      EXAMPLES.map(async (example, index) => {
        // 这里使用 scenario + report_snippet 作为向量文本
        const vectorText = await getTextEmbedding(
    ...
    const insertCount = Number(insertResult.insert_cnt) || 0;
    console.log(`已插入${insertCount}条周报示例`);
    return insertCount;
  } catch (error) {
