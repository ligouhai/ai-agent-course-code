...
export class CronJobToolService {
  ...
  private formatAtDisplay(value: unknown): string {
    if (!value) return '';
      ...
      async ({
        ...
        switch (action) {
          case 'list': {
            ...
            const lines = jobs
              .map((j: any) => {
                return `id=${j.id} type=${j.type} enabled=${j.isEnabled} running=${j.running} cron=${j.cron ?? ''} everyMs=${j.everyMs ?? ''} at=${this.formatAtDisplay(j.at)} instruction=${j.instruction ?? ''}`;
              })
              .join('\n');
          ...
          case 'add': {
            ...
            if (type === 'at') {
              ...
              return `已新增定时任务：id=${(created as any).id} type=at at=${this.formatAtDisplay((created as any).at)} enabled=${(created as any).isEnabled}`;
            }
