...
const retrieveLocalNode = async (state) => {
  console.log("---RETRIEVE_LOCAL_NODE---");
  const retrievedDocs = await retrieveRelevantContent(state.question, state.k);
  console.log(`检索到的本地文档：${retrievedDocs.length}条`);
  return {
    retrievedDocs,
    localContext: (retrievedDocs ?? []).map((doc) => doc.content).join("\n\n"),
  };
};
...
const evaluateNode = async (state) => {
  ...
  const out = await evaluator.invoke(`
    ...
    ${hasWeb ? `互联网搜索结果：${state.webContext}` : ""}
...
${hasWeb ? "" : "- web_query: 若不够，给出一个适合联网搜索的中文查询句（完整句，不用代词；为空也可）"}
  ...
  console.log(
    `${hasWeb ? "二次评估" : "评估"}结果：${out.enough} ${out.reason}`,
  );
