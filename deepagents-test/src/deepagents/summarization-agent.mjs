/*
 * @Date: 2026-07-09 11:12:22
 * @LastEditors: zhujinyi
 * @LastEditTime: 2026-07-09 11:27:32
 */
...
const workspacePath = path.join(__dirname, "workspace-summarization");
const historyPathPrefix = "/conversation_history";

const summaryPrompt = `你是对话摘要助手。请用职工问总结一下对话，包含：
1.讨论主要话题
2.达成关键结论或决定
3.继续对话所需的重要上下文
...
for (const prompt of prompts) {
  ...
  for (const file of historyFiles) {
    if (!knownHistory.has(file)) {
      ...
      console.log("已经触发摘要，历史已写入", `${historyPathPrefix}/${file}`);
    }
  }
}
